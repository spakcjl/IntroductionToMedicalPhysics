...
# This is a workaround for a specific user environment where X11 is installed in a non-standard location.
# This sets the dynamic library path so that the `imager` package can find its dependencies.
Sys.setenv(DYLD_LIBRARY_PATH = "/Users/cjlong/Desktop/Apps/XQuartz_upacked/XQuartzComponent.pkg/opt/X11/lib")
```
...
required_packages <- c("ggplot2", "dplyr", "reticulate", "patchwork", "gganimate", "gifski")
...
```{r imager_setup, include=FALSE}
# imager has a system dependency (X11), so we load it in its own chunk
...
for (i in 1:length(angles)) {
  # `imager` is helpful for rotation
  img_rot <- as.cimg(phantom_image) %>% imrotate(angles[i], interpolation = "linear")

  # Sum the columns to get the 1D projection
  projections[[i]] <- colSums(as.matrix(img_rot))
}
```
::: notes
